...
def _is_secret_configured(env_values: Dict[str, str], key: str, placeholders: set[str]) -> bool:
    ...
    """
    host = _host()
    if host._is_configured_secret(env_values.get(key, ""), placeholders):
        return True
    return host._is_configured_secret(os.environ.get(key, ""), placeholders)  # noqa: env-gate — settings UI must see env_file-injected secrets
...
def register_settings_routes(
    ...
    async def update_data_source_settings(payload: UpdateDataSourceSettingsRequest):
        ...
        if updates:
            ...
            if host_ref._is_configured_secret(finlab_token, FINLAB_TOKEN_PLACEHOLDERS):
            ...
            for env_key in ("SJ_API_KEY", "SJ_SEC_KEY"):
                ...
                if host_ref._is_configured_secret(value, SHIOAJI_KEY_PLACEHOLDERS):
